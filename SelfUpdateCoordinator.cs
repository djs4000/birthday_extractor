    ...
    internal static class SelfUpdateCoordinator
    {
        ...
        public static bool TryHandleSelfReplacement()
        {
            try
            {
                ...
                TryCleanupPreviousExecutable(currentPath);
        ...
        private static void TryCleanupPreviousExecutable(string currentPath)
        {
            try
            {
                var previous = Environment.GetEnvironmentVariable("BIRTHDAY_EXTRACTOR_PREVIOUS_EXE");
                if (string.IsNullOrWhiteSpace(previous))
                {
                    return;
                }

                Environment.SetEnvironmentVariable("BIRTHDAY_EXTRACTOR_PREVIOUS_EXE", null);

                var normalizedPrevious = Path.GetFullPath(previous);
                if (string.Equals(normalizedPrevious, currentPath, StringComparison.OrdinalIgnoreCase))
                {
                    return;
                }
                ...
                {
                    if (!File.Exists(normalizedPrevious))
                    {
                        break;
                    }
                    ...
                    try
                    {
                        File.Delete(normalizedPrevious);
                        break;
                    }
                    catch (IOException)
                    {
                ...
                if (File.Exists(normalizedPrevious))
                {
                    LogRouter.LogMessage($"Self-update: unable to remove previous executable at {normalizedPrevious}.");
                }
                else
                {
                    LogRouter.LogMessage($"Self-update: removed previous executable at {normalizedPrevious}.");
                }
            }
            catch (Exception ex)
            {
                LogRouter.LogException(ex, "Self-update: failed to clean up previous executable");
            }
        }
